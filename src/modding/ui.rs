...
use bevy::prelude::*;
use bevy_simple_text_input::{TextInputPlugin, TextInputSubmitEvent, TextInputValue};
                    ...
                    PanelBuilder::new()
                        ...
                        .build_with_children(header, |tabs| {
                            ...
                            ButtonBuilder::new("Active Modset")
                                .style(
                                    if browser_state.current_tab == ModBrowserTab::ActiveModset {
                                        ButtonStyle::Primary
                                    } else {
                                        ButtonStyle::Secondary
                                    },
...
fn handle_mod_toggles(
    ...
    for (interaction, toggle) in &mut interaction_query {
        if *interaction == Interaction::Pressed {
            if let Some(mod_ref) = mod_manager
                .available_mods
                .iter_mut()
                .find(|m| m.manifest.id == toggle.mod_id)
            {
                ...
                if mod_ref.enabled {
                    state
                        .pending_changes
                        .push(ModChange::Enable(toggle.mod_id.clone()));
                } else {
                    state
                        .pending_changes
                        .push(ModChange::Disable(toggle.mod_id.clone()));
                }

...
impl Plugin for SteamPlugin {
    fn build(&self, app: &mut App) {
        ...
        // Steam systems
        app.add_systems(Startup, (setup_steam_callbacks, initialize_stats))
            .add_systems(
                Update,
                (
...
fn sync_cloud_saves(steam: Res<SteamClient>, mut sync_events: EventWriter<CloudSaveSyncedEvent>) {
...
pub fn subscribe_to_workshop_item(steam: &SteamClient, item_id: PublishedFileId) {
    let client = &steam.0;
    let ugc = client.ugc();
...
fn initialize_stats(steam: Res<SteamClient>) {
    let client = &steam.0;
    let user_stats = client.user_stats();
...
fn setup_steam_callbacks(steam: Res<SteamClient>) {
    println!("Setting up Steam callbacks...");
    // Callbacks are handled automatically by bevy_steamworks
}

fn poll_steam_callbacks(single: Res<SingleClient>) {
    // Poll Steam for callbacks
    single.run_callbacks();
}

fn cleanup_steam(steam: Res<SteamClient>, stats: Res<SteamStats>) {

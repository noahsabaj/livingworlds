...
fn generate_clouds_internal(rng: &mut StdRng, dimensions: &MapDimensions) -> CloudSystem {
    ...
    for (layer, num_clouds, base_alpha, speed_mult) in layer_configs {
        ...
        for i in 0..num_clouds {
            ...
            // Add random offset
            let offset_x = (rng.r#gen::<f32>() * 2.0 - 1.0) * map_width * 0.05;
            let offset_y = (rng.r#gen::<f32>() * 2.0 - 1.0) * map_height * 0.05;

            let position = Vec2::new(base_x + offset_x, base_y + offset_y);

            // Random size variation
            let size = rng.gen_range(100.0..300.0);

            // Wind velocity based on layer
            let wind_speed = speed_mult * rng.gen_range(0.5..1.0);
            let velocity = Vec2::new(wind_speed, 0.0);

            // Alpha variation
            let alpha = base_alpha * rng.gen_range(0.7..1.0);
            ...
            all_clouds.push(CloudData {
                ...
                alpha,
                velocity,
                texture_index: rng.gen_range(0..5), // 5 textures per layer
            });
        }
    }
